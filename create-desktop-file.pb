...
;-┌──GUI──┐
If OpenWindow(#Window, 0, 0, w, h, "Создать desktop файл", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_MaximizeGadget | #PB_Window_MinimizeGadget | #PB_Window_ScreenCentered)
	CompilerIf  #PB_Compiler_OS = #PB_OS_Linux
	...
	w2 = w - 5
	w_field = w - b
	TextGadget(#TxtG1, 4, h_step, b - 5, FontHeight, "Имя ярлыка", #PB_Text_Border )
	...
	h_step + FontHeight
	TextGadget(#TxtG2, 4, h_step, b - 5, FontHeight, "Исполняемый файл", #PB_Text_Border)
	...
	h_step + FontHeight
	TextGadget(#TxtG3, 4, h_step, b - 5, FontHeight, "Иконка", #PB_Text_Border)
	...
	h_step + FontHeight
	TextGadget(#TxtG4, 4, h_step, b - 5, FontHeight, "Категория", #PB_Text_Border)
...
; 		AddGadgetItem(#StrG4, -1, "Game")
; 		AddGadgetItem(#StrG4, -1, "Graphics")
; 		AddGadgetItem(#StrG4, -1, "Internet")
; 		AddGadgetItem(#StrG4, -1, "AudioVideo")
; 		AddGadgetItem(#StrG4, -1, "Office")
; 		AddGadgetItem(#StrG4, -1, "System")
; 		AddGadgetItem(#StrG4, -1, "Other")
...
; 		AddGadgetItem(#StrG4, -1, "Administration")
	...
	h_step + FontHeight
	TextGadget(#TxtG5, 4, h_step, b - 5, FontHeight, "Комментарий", #PB_Text_Border)
	...
	EndIf
	h_step + FontHeight
	TextGadget(#TxtG8, b, h_step, w2 - b, FontHeight * 3, ~"Перетащи сюда\nярлык (desktop-файл)", #PB_Text_Border |  #PB_Text_Center)
	
	
; 	h_step + 40
	TextGadget(#TxtG6, 4, h_step, b - 5, FontHeight - 7, "Добавить ключи:")
	
	h_step + FontHeight
	CheckBoxGadget(#ChNoDisplay, 10, h_step, b - 15, FontHeight - 13, "NoDisplay = true")
	GadgetToolTip(#ChNoDisplay, "Не добавлять ярлык в меню программ DE")
	h_step + FontHeight
	CheckBoxGadget(#ChTerminal, 10, h_step, b - 15, FontHeight - 13, "Terminal = true")
	GadgetToolTip(#ChTerminal, "Запускать в терминале")
	
	h_step + FontHeight
	TextGadget(#TxtG7, 4, h_step, b - 5, FontHeight, "Создать в папке:", #PB_Text_Border)
	...
	If OpenPreferences(ini$)
		...
		If CountGadgetItems(#Cmb) = 0
			AddGadgetItem(#Cmb, -1, "~/Рабочий стол")
...
; 		AddGadgetItem(#Cmb, -1, "/usr/share/applications")
	...
	h_step + FontHeight
	ButtonGadget(#BtnCreate, (w - b) / 2, h_step, b, FontHeight, "Создать")
	GadgetToolTip(#BtnCreate, "Создаёт ярлык")
	ButtonGadget(#BtnINI, 4, h_step, FontHeight, FontHeight, "ini")
	ButtonGadget(#BtnOpnFile, w - b / 2, h_step, b / 2, FontHeight, ".desktop")
	GadgetToolTip(#BtnOpnFile, "Открыть текущий desktop-файл")
...
; 	EnableGadgetDrop(#StrG4, #PB_Drop_Image, #PB_Drag_Copy)
; 	EnableGadgetDrop(#StrG5, #PB_Drop_Image, #PB_Drag_Copy)
; 	EnableGadgetDrop(#Cmb, #PB_Drop_Image, #PB_Drag_Copy)
...
;- ├ Gadget
			Case #PB_Event_Gadget
				Select EventGadget()
					Case #BtnOpenDesktop
						tmp$ = OpenFileRequester("Открыть файл", AppPath$, "*.desktop|*.desktop", 0)
						If Asc(tmp$)
							AddDesktop(tmp$)
							AppPath$ = tmp$
						EndIf
					Case #BtnOpenExec
						tmp$ = OpenFileRequester("Открыть файл", ExecPath$, "Исполняемый|*", 0)
						If Asc(tmp$)
							SetGadgetText(#StrG2 , tmp$)
							ExecPath$ = tmp$
						EndIf
					Case #BtnOpenIcon
						tmp$ = OpenFileRequester("Открыть файл", iconPath$, "Значки (png, gif, svg)|*.png;*.gif;*.svg", 0)
						If Asc(tmp$)
...
; 							tmp$ = toUTF8(tmp$)
						...
						tmp$ = SaveFileRequester("Пожалуйста выберите файл для сохранения", tmp$, "*.desktop|*.desktop", 0)
						If Asc(tmp$)
...
; 							EndIf
							...
							EndIf
						Else
							If #PB_MessageRequester_Yes = MessageRequester("Предупреждение", "Вы хотите перезаписать существующий файл?", #PB_MessageRequester_YesNo | #PB_MessageRequester_Warning)
								ReadFileR(tmp$, 0)
							Else
								Continue
							EndIf
